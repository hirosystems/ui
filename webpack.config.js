...
function writeDistFileDoc(sriHash) {
  ...
  const cdnUrl = `https://unpkg.com/blockstack@${version}/dist/blockstack-ui.js`;
...
module.exports = (env, argv) => {
  ...
  const isEnvDev =
    [process.env, env.NODE_ENV, argv.mode].includes("development") ||
    env.development;
  const isEnvTest =
    [process.env, env.NODE_ENV, argv.mode].includes("test") || env.test;
  const isEnvProd =
    [process.env, env.NODE_ENV, argv.mode].includes("production") ||
    env.production;
    ...
    opts.plugins.push(
      new WebpackAssetsManifest({
        ...
        customize(entry, original, manifest, asset) {
          if (entry.value === "blockstack-ui.js") {
